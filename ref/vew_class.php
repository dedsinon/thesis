    ...
    <?php
        ...
        if($filter_by){
            ...
            if($section_id || $teacher_id ||$room_id){
                $get_schedule_sql ="SELECT * FROM schedule as sc LEFT JOIN subject as su ON sc.subject_id_fk = su.subj_id LEFT JOIN rooms AS ro ON sc.room_id_fk = ro.room_id LEFT JOIN section AS sec ON sc.section_id_fk = sec.sec_id WHERE $where_sql $where_settings_sql AND sc.school_year = '$filter_school_year' ";
                    ...
                    $.ajax({
                        ...
                        success: function(data){
                            ...
                            //$.each(data.data, function(x, obj) {
                                ...
                                $('#frm_edit_schedule .modal-header .modal-title .action-wrapper').html('<a style="font-size: 10px; margin: -2px 10px 0;" id="btn-action-disapprove" onclick="disapproveSchedule('+obj.schedule_id+');" href="javascript:;" class="btn btn-danger">Disapprove Schedule Now</a>');
...
function disapproveSchedule(schedule_id){
    if (confirm('Are you sure you want to Disapprove Schedule #'+schedule_id+'?')) {
        $.ajax({
            type: "POST",
    ...
    <div class="modal fade" id="myModalEdit" role="dialog">
        ...
        <div class="modal-content">
            <div class="modal-header">
              ...
              <h4 class="modal-title">Schedule Confirmation</h4>
            </div>
