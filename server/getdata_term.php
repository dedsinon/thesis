...
if (!empty($_POST["e_term_grading"])){
	...
	$query = "SELECT * FROM schedule as sc LEFT JOIN subject as subj ON sc.subject_id_fk = subj.subj_id LEFT JOIN section as sec ON sc.section_id_fk = sec.sec_id WHERE term_grading = '".$term."' AND sc.school_year = '$activated_year' AND course_strand = 'Course' ";
